...
def display_product(result):
    ...
    product = {k.strip().lstrip("\ufeff"): v for k, v in product.items()}
    ...
    if product["stock_status"] == "残りわずか":
        st.warning(ct.STOCK_WARNING_MESSAGE, icon=ct.WARNING_ICON)
    elif product["stock_status"] == "なし":
        st.error(ct.STOCK_ERROR_MESSAGE, icon=ct.ERROR_ICON)
